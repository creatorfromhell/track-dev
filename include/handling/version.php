			...
			if(!VersionFunc::exists(cleanInput($_POST['version-name']))) {
				if(isset($_POST['status']) && trim($_POST['status']) != "") {
					if(isset($_POST['version-type']) && trim($_POST['version-type']) != "") {
						...
						echo '<script type="text/javascript">';
						echo 'showMessage("error", "'.$formatter->replaceShortcuts(((string)$languageinstance->site->forms->version->notype)).'");';
						echo '</script>';
					}
				} else {
					echo '<script type="text/javascript">';
				...
				echo '<script type="text/javascript">';
				echo 'showMessage("error", "'.$formatter->replaceShortcuts(((string)$languageinstance->site->forms->version->taken)).'");';
				echo '</script>';
			}
		} else {
			echo '<script type="text/javascript">';
