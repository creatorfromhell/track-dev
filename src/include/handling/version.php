...
if(isset($_POST['add-version'])) {
	if(isset($_POST['project']) && trim($_POST['project']) != "") {
		if(isset($_POST['version-name']) && trim($_POST['version-name']) != "") {
			if(!hasValues("versions", " WHERE version_name = '".cleanInput($_POST['version-name'])."'")) {
				if(isset($_POST['status']) && trim($_POST['status']) != "") {
					if(isset($_POST['version-type']) && trim($_POST['version-type']) != "") {
						if(isset($_POST['captcha']) && trim($_POST['captcha']) != '' && checkCaptcha(cleanInput($_POST['captcha']))) {
							...
							VersionFunc::addVersion($version, $project, $status, $due, '0000-00-00', $type);
						...
						echo '<script type="text/javascript">';
						echo 'showMessage("error", "'.$formatter->replaceShortcuts($language_manager->getValue($language, "site->forms->version->notype")).'");';
						echo '</script>';
					}
				} else {
					echo '<script type="text/javascript">';
				...
				echo '<script type="text/javascript">';
				echo 'showMessage("error", "'.$formatter->replaceShortcuts($language_manager->getValue($language, "site->forms->version->taken")).'");';
				echo '</script>';
			}
		} else {
			echo '<script type="text/javascript">';
...
if(isset($_POST['edit-version'])) {
	if(isset($_POST['id']) && trim($_POST['id']) != "") {
		$details = VersionFunc::versionDetails(cleanInput($_POST['id']));
		if(isset($_POST['project']) && trim($_POST['project']) != "") {
			if(isset($_POST['version-name']) && trim($_POST['version-name']) != "") {
				if($details['name'] == cleanInput($_POST['version-name']) || $details['name'] != cleanInput($_POST['version-name']) && !hasValues("versions", " WHERE version_name = '".cleanInput($_POST['version-name'])."'")) {
					if(isset($_POST['status']) && trim($_POST['status']) != "") {
						if(isset($_POST['version-type']) && trim($_POST['version-type']) != "") {
							if(isset($_POST['captcha']) && trim($_POST['captcha']) != '' && checkCaptcha(cleanInput($_POST['captcha']))) {
								...
								VersionFunc::editVersion($id, $version, $project, $status, $due, '0000-00-00', $type);
								destroySession("userspluscaptcha");
							} else {
								echo '<script type="text/javascript">';
								echo 'showMessage("error", "'.$formatter->replaceShortcuts($language_manager->getValue($language, "site->forms->invalidcaptcha")).'");';
