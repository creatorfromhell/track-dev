    ...
    if(isset($_POST['node']) && trim($_POST['node']) != '') {
        if(isset($_POST['description']) && trim($_POST['description']) != '') {
            if(isset($_POST['captcha']) && trim($_POST['captcha']) != '' && check_captcha(StringFormatter::clean_input($_POST['captcha']))) {
                $node = StringFormatter::clean_input($_POST['node']);
                $description = StringFormatter::clean_input($_POST['description']);
...
if(isset($_POST['edit-permission'])) {
    if(isset($_POST['id']) && trim($_POST['id']) != '' && has_values("nodes", " WHERE id = '".StringFormatter::clean_input($_POST['id'])."'")) {
        if(isset($_POST['node']) && trim($_POST['node']) != '') {
            if(isset($_POST['description']) && trim($_POST['description']) != '') {
                if(isset($_POST['captcha']) && trim($_POST['captcha']) != '' && check_captcha(StringFormatter::clean_input($_POST['captcha']))) {
                    ...
                    echo '<script type="text/javascript">';
                    echo 'showMessage("error", "'.$language_manager->get_value($language, "site->forms->invalid->captcha").'");';
                    echo '</script>';
                }
            } else {
                die("Invalid description entered.");
        ...
        echo '<script type="text/javascript">';
        echo 'showMessage("error", "'.$language_manager->get_value($language, "site->forms->invalid->id").'");';
        echo '</script>';
    }
}